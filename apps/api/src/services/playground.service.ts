...
import { CreateTargetGroupCommand, ElasticLoadBalancingV2Client, RegisterTargetsCommand } from "@aws-sdk/client-elastic-load-balancing-v2"
...
export async function createTargetGroupForPlayground() {
    ...
    if (elbResponse.TargetGroups == undefined || elbResponse.TargetGroups.length < 0) throw new CustomError(400, "Failed to create a target group")
    if (elbResponse.TargetGroups[0].TargetGroupArn == undefined) throw new CustomError(400, "Failed to create a target group")
