...
const Editor: React.FC = () => {
  const { activeFile, fileContents, setFileContent, loading, setLoading } = useIDEStore();
  ...
  useEffect(() => {
    const loadFileContent = async () => {
      if (activeFile && !fileContents[activeFile]) {
        setLoading(true);
        try {
          const content = await fetchFileContent(activeFile);
    ...
    loadFileContent();
  }, [activeFile, fileContents, setFileContent, setLoading]);
  ...
  const handleChange = useCallback(
    async (value: string) => {
      if (activeFile) {
        ...
        try {
          await saveFileContent(activeFile, value);
        } catch (error) {
  ...
  if (!activeFile) {
    return (
      <div className="flex-1 bg-ide-editor flex flex-col">
